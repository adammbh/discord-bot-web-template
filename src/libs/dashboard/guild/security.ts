...
export async function fetchGuildSecurity(guildId: string): Promise<SecuritySettings> {
    ...
    const response = await fetch(`https://api.yourbot.bot/security`, {
        headers: {
...
export async function updateGuildSecurity(
    guildId: string,
    settings: SecurityUpdateRequest
    ...
    const response = await fetch(`https://api.yourbot.bot/update/security`, {
        method: "POST",
        headers: {
            Authorization: `Bearer ${token}`,
            "X-GUILD-ID": guildId,
