...
const API_TOKEN = process.env.API_TOKEN || "xxx"
...
export async function GET(request: NextRequest) {
    ...
    try {
        const response = await fetch(`https://api.yourbot.bot/levels?_=${Date.now()}`, {
            headers: {
        ...
        if (!response.ok) {
            throw new Error(
                `API responded with status: ${response.status}. If you are a user, please contact the @yourtag`
            )
        }
