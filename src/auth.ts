            ...
            token: {
                ...
                async request(context: any) {
                    ...
                    const response = await fetch(
                        `https://ws.audioscrobbler.com/2.0?method=auth.getSession&api_key=${process.env.LASTFM_API_KEY}&token=${context.params.code}&format=json`
                    )
    ...
    callbacks: {
        async signIn({ account, user, profile }) {
            if (account?.provider === "discord") {
                try {
                    const response = await fetch("https://api.your.bot/login", {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
                            "X-Special-Auth": "your-auth-key",
                            Origin: "https://your.bot"
                        },
                        body: JSON.stringify({
                            user_id: profile?.id ?? "",
            ...
            if (account?.provider === "spotify") {
                ...
                try {
                    ...
                    const response = await fetch("https://api.your.bot/spotify/auth", {
                        method: "POST",
                        headers: {
