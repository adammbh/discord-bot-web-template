        ...
        <>
                ...
                <>
                    ...
                    <div className="fixed inset-0 z-[50001] flex items-center justify-center">
                        <motion.div
                            ...
                            className="bg-yourbot-200 border border-yourbot-card-border p-6 rounded-xl shadow-lg max-w-md mx-4">
                            <div className="flex justify-between items-start mb-4">
                                <div className="bg-white/10 p-2 rounded-lg">
                                    <Sparkles className="w-6 h-6 text-yourbot-pink" />
                                </div>
                                <button
                                    onClick={dismissPopup}
                            ...
                            <div className="flex gap-3">
                                <button
                                    ...
                                    className="flex-1 bg-yourbot-pink text-white px-4 py-2 rounded-lg font-medium hover:bg-yourbot-pink/90 transition-colors">
                                    Join Beta
                                </button>
                                <button
                                    onClick={dismissPopup}
            ...
            <div className="flex flex-row items-center justify-center space-x-4">
                <div className="block lg:hidden">
                    <MdMenu
                        size={32}
                        className="hover:cursor-pointer hover:text-yourbot-pink"
                        onClick={() => setIsBurgerMenuOpen(!isBurgerMenuOpen)}
                    />
                </div>
                <Link
                    href="/purchase"
                    className="text-zinc-500 hover:text-yourbot-pink transition-colors">
                ...
                <button
                    ...
                    <span className="font-normal hidden sm:inline">Join Beta</span>
                    <span className="font-normal sm:hidden">Beta</span>
                </button>
            </div>
        </>
    )
...
const BurgerMenu = ({ onClose }: { onClose: () => void }) => {
    ...
    const routes = useMemo(
        () => [
            ...
            {
                label: "Embeds",
                destination: "https://embeds.yourbot.bot",
                isActive: pathname == "https://embeds.yourbot.bot"
            },
            {
                label: "Docs",
                destination: "https://docs.yourbot.bot",
                isActive: pathname == "https://docs.yourbot.bot"
            },
            {
                label: "Invite",
                destination: "/invite",
        ...
        <>
            <div className="fixed inset-0 z-[9999999999] flex items-center justify-center">
                <motion.div
                    ...
                    className="bg-yourbot-200 border -mt-40 border-yourbot-card-border w-[90%] px-2 rounded-xl shadow-lg">
                    <div className="flex flex-row justify-between items-center gap-6 pt-10 px-4">
                        ...
                        <CgClose
                            size={24}
                            className="ml-auto hover:cursor-pointer hover:text-yourbot-pink"
                            onClick={onClose}
                        />
                    </div>
                        ...
                        {routes.map(route => {
                            return (
                                <Link
                                    ...
                                    key={route.label}
                                    onClick={onClose}
                                    className={`flex items-center h-14 bg-yourbot-300 rounded-md ${
                                        route.isActive
                                            ? "text-yourbot-pink bg-yourbot-200"
                                            : "text-yourbot-700 hover:bg-yourbot-dim hover:text-white"
                                    }`}>
